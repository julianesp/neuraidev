...
const CartSummarySection = ({ subtotal, itemCount, items }) => {
  const handleSendToWhatsApp = () => {
    ...
    items.forEach((item, index) => {
      ...
      message += `   💰 Precio: $${precio.toLocaleString()} COP\n`;
      message += `   📦 Cantidad: ${item.cantidad}\n`;
      message += `   🏷️ Categoría: ${item.producto.categoria.replace("-", " ")}\n`;
      message += `   💵 Subtotal: $${subtotalItem.toLocaleString()} COP\n\n`;
    });

    message += `💳 *TOTAL DEL PEDIDO: $${subtotal.toLocaleString()} COP*\n\n`;
    message += "📞 Por favor, envíame el enlace de pago para completar la compra.\n\n";
    message += "¡Gracias! 😊";

    const encodedMessage = encodeURIComponent(message);
    const whatsappUrl = `https://wa.me/573174503604?text=${encodedMessage}`;
