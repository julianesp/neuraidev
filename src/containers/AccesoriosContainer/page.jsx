...
const AccesoriosContainer = ({
  ...
  const cambiarAccesorio = useCallback((nuevoAccesorio) => {
    if (!nuevoAccesorio) {
    ...
    const nuevosOtros = todosAccesorios.filter(
      (item) => item.id !== nuevoAccesorio.id || item.nombre !== nuevoAccesorio.nombre
    );
  ...
  const navegarSlide = useCallback((direccion) => {
    if (!accesorio?.imagenes?.length || accesorio.imagenes.length <= 1) return;

    setMainSlideIndex(prevIndex => {
      if (direccion === 'next') {
        return prevIndex === accesorio.imagenes.length - 1 ? 0 : prevIndex + 1;
      } else {
        return prevIndex === 0 ? accesorio.imagenes.length - 1 : prevIndex - 1;
      }
    });
  }, [accesorio?.imagenes?.length]);

  const nextMainSlide = useCallback((e) => {
    e.preventDefault();
    navegarSlide('next');
  }, [navegarSlide]);

  const prevMainSlide = useCallback((e) => {
    e.preventDefault();
    navegarSlide('prev');
  }, [navegarSlide]);
  ...
  const { whatsappUrl, tieneImagenes, imagenPrincipal } = useMemo(() => {
    if (!accesorio) return { whatsappUrl: "", tieneImagenes: false, imagenPrincipal: null };
    ...
    if (!imagenPrincipal && tieneImagenes) {
      ...
      imagenPrincipal = typeof primeraImagen === "object" && primeraImagen.url 
        ? primeraImagen.url 
        : primeraImagen;
    }
    ...
    return (
      <div className="max-w-6xl mx-auto p-4 bg-white/30 backdrop-blur-md rounded-lg shadow-lg">
        <div className="text-center py-12">
          ...
          <p className="text-gray-600 mt-2">Intenta recargar la página o verifica la conexión.</p>
        </div>
      </div>
    );
  }
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
        ...
        >
          <div className="h-full w-full relative overflow-hidden rounded-lg bg-white/10">
            {tieneImagenes ? (
              accesorio.imagenes.map((imagen, index) => {
                const imagenUrl = typeof imagen === "object" && imagen.url 
                  ? imagen.url 
                  : imagen;
      ...
      {otrosAccesorios?.length > 0 && (
        <section className="mt-12" aria-labelledby="related-products-heading">
          <h2 id="related-products-heading" className="text-2xl font-bold mb-6 text-center text-gray-900 dark:text-white">
            Otros accesorios que te pueden interesar
          </h2>

          <div className={`${styles.otrosAccesoriosGrid} grid gap-4 ${
            isMobile ? 'grid-cols-2' : 'grid-cols-3 md:grid-cols-4'
          }`}>
            {otrosAccesorios.map((item, itemIndex) => {
              const itemImageUrl = item.imagenPrincipal || 
