...
async function sendEmailNotification(subject, htmlContent) {
  try {
    ...
    log(`📧 Enviando email a ${adminEmail}...`);

    // Usar el API de Resend (gratuito hasta 100 emails/día)
    const resendApiKey = process.env.RESEND_API_KEY;

    if (!resendApiKey) {
      log("⚠️ RESEND_API_KEY no configurado. Email omitido.");
      return false;
    }
    ...
    const response = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: 'Neurai Dev <ventas@neurai.dev>',
        to: [adminEmail],
        subject: subject,
        html: htmlContent,
      }),
    ...
    if (!response.ok) {
      ...
      logError('Error al enviar email:', errorData);
      return false;
    }
    ...
    logError('❌ Error en sendEmailNotification:', error);
    return false;
  }
}
