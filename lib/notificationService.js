...
export async function notifyNewSale(orderData, transactionData) {
  try {
    ...
    // metadata puede llegar como string JSON (D1/Cloudflare) o como objeto.
    // Si no se normaliza, orderData.metadata?.productos queda undefined y los
    // productos salen como "No disponible".
    ...
    const productos = metadata?.productos || orderData.productos || orderData.items || [];
    let productosTexto = '';
    let productosHtml = '';
    ...
    const clienteNombre = orderData.customer_name ||
                          ...
                          'Cliente';
    const clienteTelefono = orderData.customer_phone || metadata?.customer_phone || 'No especificado';
    const clienteDireccion = orderData.customer_address || orderData.direccion_envio || metadata?.customer_address || 'No especificado';
    const clienteCiudad = orderData.customer_city || metadata?.customer_city || '';
    const clienteRegion = orderData.customer_region || metadata?.customer_region || '';
...
/**
 * Envía una notificación cuando un cliente califica el servicio técnico / formateo.
 * La calificación queda pendiente de moderación hasta que el admin la publique.
 * @param {Object} data - { nombre, email, calificacion, mensaje }
 */
export async function notifyNuevaCalificacion(data) {
  try {
    const botToken = process.env.TELEGRAM_BOT_TOKEN;
    const chatId = process.env.TELEGRAM_ADMIN_CHAT_ID;
    ...
    const calif = parseInt(data.calificacion) || 0;
    const estrellas = calif > 0 ? "⭐".repeat(calif) : "Sin calificación";

    const mensaje = `
⭐ <b>¡NUEVA CALIFICACIÓN DE FORMATEO!</b>

👤 <b>Cliente:</b> ${data.nombre || "No especificado"}
📧 <b>Email:</b> ${data.email || "No especificado"}
⭐ <b>Calificación:</b> ${estrellas} (${calif}/5)

💬 <b>Comentario:</b>
${data.mensaje || "(sin comentario)"}

📌 <b>Estado:</b> Pendiente de aprobación
👉 Revísala y publícala desde el panel: /dashboard/calificaciones
...
`.trim();
    ...
    console.error("[NOTIF ERROR] Error al notificar nueva calificación:", error);
    return false;
  }
}

/**
 * Envía una notificación cuando alguien deja un comentario nuevo en el blog
 * (queda pendiente de moderación hasta que el admin lo apruebe).
 * @param {Object} commentData - { postSlug, userName, content }
 */
export async function notifyNewBlogComment(commentData) {
  try {
    const botToken = process.env.TELEGRAM_BOT_TOKEN;
    const chatId = process.env.TELEGRAM_ADMIN_CHAT_ID;
    ...
    // Escapar caracteres HTML del contenido para parse_mode HTML
    ...
    const mensaje = `
💬 <b>¡NUEVO COMENTARIO EN EL BLOG!</b>

👤 <b>Autor:</b> ${escapeHtml(commentData.userName) || "Anónimo"}
📝 <b>Artículo:</b> ${escapeHtml(commentData.postSlug)}

🗨️ <b>Comentario:</b>
${escapeHtml(commentData.content)}

⏳ <b>Estado:</b> Pendiente de aprobación
🔗 Modera en: /dashboard/comentarios
...
`.trim();
    ...
    console.error("[NOTIF ERROR] Error al notificar nuevo comentario:", error);
    return false;
  }
}

/**
 * Envía una notificación cuando un usuario escribe en el chat de soporte.
 * @param {Object} data - { nombre, asunto, contenido }
 */
export async function notifyNewChatMessage(data) {
  try {
    ...
    const mensaje = `
💬 <b>¡NUEVO MENSAJE EN EL CHAT DE SOPORTE!</b>

👤 <b>Cliente:</b> ${escapeHtml(data.nombre) || "Anónimo"}
📌 <b>Asunto:</b> ${escapeHtml(data.asunto) || "Consulta general"}

🗨️ <b>Mensaje:</b>
${escapeHtml(data.contenido)}

🔗 Responde en: /dashboard/chat
...
`.trim();
    ...
    console.error("[NOTIF ERROR] Error al notificar mensaje de chat:", error);
    return false;
  }
}
