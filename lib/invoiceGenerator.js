    ...
    try {
      ...
      items.forEach((item, index) => {
        ...
        doc
          .fontSize(9)
          .fillColor(textColor)
          .text(item.name || 'Producto', colProduct + 10, yPosition, {
            width: 260,
            ellipsis: true,
          })
          .text(String(item.quantity || 1), colQuantity, yPosition)
          .text(formatCurrency(item.price || 0), colPrice, yPosition)
...
export async function createInvoiceRecord(supabase, orderData, transactionData = null) {
  try {
    ...
    const invoiceData = {
      ...
      order_reference: orderData.invoice || orderData.order_reference,
      customer_name: orderData.customer_name || 'Cliente',
      customer_email: orderData.customer_email,
      customer_phone: orderData.customer_phone || null,
      customer_type_doc: orderData.customer_type_doc || null,
      customer_number_doc: orderData.customer_number_doc || null,
      customer_address: orderData.customer_address || null,
      customer_city: orderData.customer_city || null,
      customer_region: orderData.customer_region || null,
      transaction_id: transactionData?.id || orderData.transaction_id || null,
      payment_method: transactionData?.payment_method_type || orderData.payment_method || null,
      items: items,
      subtotal: subtotal,
      tax: tax,
      total: total,
