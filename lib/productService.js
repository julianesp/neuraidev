...
export async function getProductById(productId, useServerClient = false) {
  try {
    ...
    if (error) {
      console.error('[ProductService] Error al obtener producto:', {
...
export async function getProductsByCategory(categoria, limit = null, useServerClient = false) {
  try {
    ...
    let query = supabase
      ...
      // Mostrar todos los productos, incluyendo los sin stock
    ...
    if (error) {
      console.error('[ProductService] Error al obtener productos por categoría:', {
        message: error.message,
        details: error.details,
        hint: error.hint,
        code: error.code,
...
export async function getFeaturedProducts(limit = 10, useServerClient = false) {
  try {
    ...
    const { data, error } = await supabase
      ...
      .select('*')
      .eq('destacado', true)
      // Mostrar todos los productos destacados, incluyendo los sin stock
      .order('created_at', { ascending: false })
      .limit(limit);

    if (error) {
      console.error('[ProductService] Error al obtener productos destacados:', {
        message: error.message,
        details: error.details,
        hint: error.hint,
        code: error.code
...
export async function getRelatedProducts(productId, categoria, limit = 8, useServerClient = false) {
  try {
    ...
    const { data, error } = await supabase
      ...
      .eq('categoria', categoria)
      .neq('id', productId)
      // Mostrar todos los productos relacionados, incluyendo los sin stock
      .order('created_at', { ascending: false })
      .limit(limit);

    if (error) {
      console.error('[ProductService] Error al obtener productos relacionados:', {
        message: error.message,
        details: error.details,
        hint: error.hint,
        code: error.code,
...
export async function checkProductStock(productId, useServerClient = false) {
  try {
    ...
    if (error) {
      console.error('[ProductService] Error al verificar stock:', {
        message: error.message,
        details: error.details,
        hint: error.hint,
        code: error.code,
...
export async function decrementProductStock(productId, quantity) {
  try {
    ...
    if (fetchError) {
      console.error('[ProductService] Error al obtener producto para descuento:', {
    ...
    if (updateError) {
      console.error('[ProductService] Error al actualizar stock:', {
...
export async function searchProducts(searchTerm, limit = 20, useServerClient = false) {
  try {
    ...
    const { data, error } = await supabase
      ...
      // Mostrar todos los productos en búsqueda, incluyendo los sin stock
      .limit(limit);

    if (error) {
      console.error('[ProductService] Error en búsqueda:', {
        message: error.message,
        details: error.details,
        hint: error.hint,
        code: error.code,
...
export async function getAllProducts(page = 1, pageSize = 50, useServerClient = false) {
  try {
    ...
    if (error) {
      console.error('[ProductService] Error al obtener todos los productos:', {
        message: error.message,
        details: error.details,
        hint: error.hint,
        code: error.code,
