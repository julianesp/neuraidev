/**
 * lib/db.js - Cliente Cloudflare D1
 *
 * Expone un query builder compatible con la API de Supabase para facilitar
 * las consultas sin cambiar el código existente.
 */
...
class D1QueryBuilder {
  ...
  // Implementa .or('col1.ilike.%val%,col2.ilike.%val%') — sintaxis Supabase
  or(filterString) {
    ...
    if (clauses.length > 0) {
      this._orGroups.push(`(${clauses.join(' OR ')})`);
    }
    return this;
  }
  ...
  eq(col, val) {
    ...
    this._params.push(typeof val === 'boolean' ? (val ? 1 : 0) : val);
    return this;
  }
  ...
  _buildSQL() {
    ...
    const allConditions = [...this._conditions, ...this._orGroups];
  ...
  _buildCountSQL() {
    let sql = `SELECT COUNT(*) as _count FROM ${this.table}`;
    const allConditions = [...this._conditions, ...this._orGroups];
    if (allConditions.length > 0) {
      sql += ' WHERE ' + allConditions.join(' AND ');
    }
  ...
  async _execute() {
    ...
    try {
      ...
      if (this._count) {
        const countResult = await d1Select(this._buildCountSQL(), this._params);
        const count = countResult[0]?._count ?? 0;
...
// Aliases de compatibilidad — apuntan al mismo cliente D1
export const getSupabaseServerClient = getDbClient;
export const getSupabaseClient = getDbClient;
export const getSupabaseBrowserClient = getDbClient;
