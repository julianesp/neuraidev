...
export function getSupabaseClient() {
  if (!supabase) {
    ...
    if (!supabaseUrl || !supabaseKey) {
      console.error("[db.js] Missing Supabase credentials:", {
        hasUrl: !!supabaseUrl,
        hasKey: !!supabaseKey,
      });
      throw new Error(
    ...
    try {
      supabase = createClient(supabaseUrl, supabaseKey, {
        ...
        global: {
          headers: {
            'x-client-info': 'neurai-dev',
          },
        },
      });
      console.log("[db.js] Supabase server client initialized successfully");
    } catch (error) {
      console.error("[db.js] Failed to create Supabase client:", error);
      throw error;
    }
  }
  return supabase;
...
export function getSupabaseBrowserClient() {
  if (!supabaseBrowser) {
    ...
    if (!supabaseUrl || !supabaseKey) {
      console.error("[db.js] Missing Supabase browser credentials:", {
        hasUrl: !!supabaseUrl,
        hasKey: !!supabaseKey,
      });
      throw new Error(
    ...
    try {
      supabaseBrowser = createClient(supabaseUrl, supabaseKey, {
        ...
        global: {
          headers: {
            'x-client-info': 'neurai-dev-browser',
          },
        },
      });
      console.log("[db.js] Supabase browser client initialized successfully");
    } catch (error) {
      console.error("[db.js] Failed to create Supabase browser client:", error);
      throw error;
    }
  }
