...
export default function AdminBlume() {
  ...
  async function fetchPublicaciones() {
    try {
      const res = await fetch("/api/blume");
  ...
  async function handleEliminar(pub) {
    ...
    try {
      ...
      const res = await fetch(`/api/blume?${params}`, { method: "DELETE" });
        ...
        <div className="mb-8 mt-12 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-1 text-sm">
              Gestiona las publicaciones de Blume
            </p>
          </div>
        </div>
